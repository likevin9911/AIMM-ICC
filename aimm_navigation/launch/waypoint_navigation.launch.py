...
def generate_launch_description():
    # 1) Point to Nav2’s built-in navigation_launch.py
    pkg_nav2nav = get_package_share_directory('nav2_navigation')
    nav2_nav_launch = os.path.join(
        pkg_nav2nav, 'launch', 'navigation_launch.py'
    )

    # 2) Your own params file
    pkg_local = get_package_share_directory('localization_launch')
    my_params = os.path.join(pkg_local, 'config', 'nav2_params.yaml')
